...
# Crear un router para la ruta /followers
SEARCH_FOLLOWERS = APIRouter()


@SEARCH_FOLLOWERS.get("/followers",
                      response_description="List all followers")
def search_following(request: Request, authentication: str = Header(...),
                     limit=15, page=1, search_param=''):
    """
    Endpoint para buscar usuarios que siguen al usuario autenticado.
    ...
    Returns:
        dict: Diccionario que contiene datos de usuarios seguidores y
                    ...
                    '$match': {
                        '$or': [
                            {'user1_id': ObjectId(user_id),
                             'follow_back': True},
                            {'user2_id': ObjectId(user_id), 'follow': True}
                        ]
                    }
                },
                {
